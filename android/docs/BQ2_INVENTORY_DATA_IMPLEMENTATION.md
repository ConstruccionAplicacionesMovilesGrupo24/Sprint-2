...
- **Other:** authenticated requests, `AppContainer` wiring and tests.

The `InventoryViewModel`, the Inventory and Empty Inventory screens, and their tests belong to Person B (Natalia) and are not part of this change.
...
| `feature/inventory/data/local/ExpiringInventoryCache.kt` | `ExpiringInventoryCache`, `CachedExpiringInventory` | Cache interface, so repository tests can use a fake. | Offline cache |
...
| `gradle/libs.versions.toml`, `app/build.gradle.kts` (modified) | `androidx-room-testing`, `androidTest` assets | Adds `room-testing` (Room 2.8.5) and exposes `app/schemas/` to migration tests. | DAO and migration tests |
...
- **Token exposure:**
  ...
  - Tests compare the header with `assertTrue(... == ...)`, so a failure message never prints it.
...
- **Schemas:** KSP generated `app/schemas/.../2.json`; `1.json` is unchanged. The schema folder is added to the `androidTest` assets so `MigrationTestHelper` can validate the migration.
...
| --- | --- | --- |
| `InventoryMappersTest` | 9 | All fields mapped; backend order kept; inactive items excluded; `-1` and `4` excluded while `0` and `3` kept; empty list valid; blank or invalid values and duplicate ids reject the response; entity round trip with `priorityIndex`; calendar validation. |
| `OfflineFirstInventoryRepositoryTest` | 11 | Uses a fake API, a fake cache and a fixed clock. Covers: success written to cache; empty success replaces cache; connectivity failure uses cache, including a cached empty list; 5xx uses cache; connectivity failure without cache is `BackendUnavailable`; missing session is `Unauthorized` with no backend call; 401 is `Unauthorized` and clears the cache; 404 is a typed error without cache; invalid response keeps the cache; `clearCache`. |
| `InventoryApiContractTest` | 2 | MockWebServer: `GET`, path `/api/v1/inventory/expiring`, `withinDays=3`, `Authorization: Bearer …`, decoding with unknown fields ignored; a missing required field gives `InvalidResponse`. |
| `SessionAuthorizationHeaderProviderTest` | 3 | Header built from the access token only; `null` without a session or with a blank token. |

**Instrumented (`app/src/androidTest`)**

| Test class | Tests | Purpose |
| --- | --- | --- |
| `RoomInventoryCacheTest` | 6 | In-memory Room: DAO order by `priorityIndex`; storage and read with timestamp; replacement of the previous result and timestamp; empty result cached; `null` before the first sync and after clear; clear keeps other caches' metadata. |
| `DatabaseMigrationTest` | 2 | `MigrationTestHelper` checks that version 1 → 2 keeps `cache_metadata` rows and matches the exported schema; `CampusMealDatabase.create()` opens a version 1 file, which proves the migration is registered. |
...
| `./gradlew :app:testDebugUnitTest` | BUILD SUCCESSFUL. 36 tests, 0 failures: 25 new BQ2 tests plus 11 existing. |
...
| `./gradlew :app:connectedDebugAndroidTest` | BUILD SUCCESSFUL on `Tusky_API_36` (API 36). 10 tests, 0 failures: `RoomInventoryCacheTest` 6, `DatabaseMigrationTest` 2, and the existing `CampusMealAppTest` and `CampusMealDatabaseTest`. |

Static checks:
- **Build:** no `org.jetbrains.kotlin.android` or kapt in the build configuration; Room uses `ksp(...)`; `room-testing` uses `version.ref = "room"`; no pinned version changed.
...
- **Ownership:** Natalia's files (`MainActivity`, `CampusMealApp`, theme, navigation, `UiState`, manifest, smoke tests) are unmodified.
